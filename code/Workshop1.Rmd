...
Ans: Larger is the size of the engine lower is the fuel efficiency i.e cars with large engines will consume more fuel. 
...
Ans: The size or alpha argument would be valid for continuous data.
...
Ans: nrow and ncol specifies the number of rows and columns as in whether we want each plot in that facet in one or more than one rows. Other options that control the layout of the panel are shrink which will shrink the plots and help you adjust more data on the plot. labeller allows you to ustomize the labels. 

```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_wrap(~ class, nrow = 2, shrink = TRUE, labeller = "label_value")
```
...
Ans: The method argument decides how to smoothen the line.  
...
Ans: SE displays the confidence interval of the smoothened line so depending on whether we put SE = FALSE or SE = TRUE it will not display or display respectively.

# Exercise:
1.  What geom would you use to draw a line chart? A boxplot? A histogram? An area chart?
Ans: To draw a line chart I would use geom_line, for a box plot I would use geom_box and for a histogram I would use geom_hist and for an area chart I would use geom_area
...
```
Ans: No these plots would not look differnt as the arguments provided in ggplot () is same as arguments provided in geom_point () and geom_smooth()
...
Ans: We specified y axis as we need to plot the cut vs teh frequency had we not specified the y axis = frequency it woudl have shown us count which is basically teh number of times the specific "cut" has ocurred in that dataframe. 
